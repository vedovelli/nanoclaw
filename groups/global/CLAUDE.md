...
**Exception: scheduled tasks.** When the prompt starts with `[SCHEDULED TASK`, never send an acknowledgment. The user did not send a message, so there is nothing to acknowledge. Only send messages if there is something meaningful to report.
